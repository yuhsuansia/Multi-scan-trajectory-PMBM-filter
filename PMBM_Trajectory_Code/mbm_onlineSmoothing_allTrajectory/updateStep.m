...
trajectoryNewMBM = repmat(trajectoryNewMBM,(m+1)*nb,1);
...
for i = 1:nb
    ...
    temp = 1-MB_birth(i).r+MB_birth(i).r*(1-Pd);
    trajectoryNewMBM((i-1)*(m+1)+1).c = -log(temp);
    trajectoryNewMBM((i-1)*(m+1)+1).r = MB_birth(i).r*(1-Pd)/temp;
    trajectoryNewMBM((i-1)*(m+1)+1).xpre = MB_birth(i).x;
    trajectoryNewMBM((i-1)*(m+1)+1).Ppre = MB_birth(i).P;
    trajectoryNewMBM((i-1)*(m+1)+1).x = MB_birth(i).x;
    trajectoryNewMBM((i-1)*(m+1)+1).P = MB_birth(i).P;
    trajectoryNewMBM((i-1)*(m+1)+1).l = [t,0];
    if nupd==0
        trajectoryNewMBM((i-1)*(m+1)+1).a = i;
    else
        trajectoryNewMBM((i-1)*(m+1)+1).a = trajectoryUpdMBM(end).a+i;
    end
    trajectoryNewMBM((i-1)*(m+1)+1).beta = MB_birth(i).beta;
    trajectoryNewMBM((i-1)*(m+1)+1).epsilon = MB_birth(i).epsilon;
    ...
    for j = 1:m
        ...
        if ck/(ck+lambda_fa) < model.threshold && unusedMeas(j)==true
            idx_remain((i-1)*(m+1)+1+j) = false;
        else
            usedMeas(j) = true;
            trajectoryNewMBM((i-1)*(m+1)+1+j).c =-log(ck);
            trajectoryNewMBM((i-1)*(m+1)+1+j).r = 1;
            trajectoryNewMBM((i-1)*(m+1)+1+j).xpre = MB_birth(i).x;
            trajectoryNewMBM((i-1)*(m+1)+1+j).Ppre = MB_birth(i).P;
            trajectoryNewMBM((i-1)*(m+1)+1+j).x = MB_birth(i).x + Kk(:,:,i)*v;
            trajectoryNewMBM((i-1)*(m+1)+1+j).P = Pk(:,:,i);
            % Otherwise, add the index of the measurement at current scan
            trajectoryNewMBM((i-1)*(m+1)+1+j).l = [t,j];
            if nupd==0
                trajectoryNewMBM((i-1)*(m+1)+1+j).a = i;
            else
                trajectoryNewMBM((i-1)*(m+1)+1+j).a = trajectoryUpdMBM(end).a+i;
            end
            trajectoryNewMBM((i-1)*(m+1)+1+j).beta = MB_birth(i).beta;
            trajectoryNewMBM((i-1)*(m+1)+1+j).epsilon = t;
        end
    end
end
