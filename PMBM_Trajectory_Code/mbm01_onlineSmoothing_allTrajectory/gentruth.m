...
% We consider an MB birth
nb = 1;
model.rb = zeros(nb,1);
model.xb = zeros(4,nb);
model.Pb = zeros(4,4,nb);
for i = 1:nb
    model.rb(i) = 0.05; % expect one new target to arrive every 20 scans on average
    model.xb(:,i) = zeros(4,1);
    model.Pb(:,:,i) = diag([100 1 100 1].^2);
end
