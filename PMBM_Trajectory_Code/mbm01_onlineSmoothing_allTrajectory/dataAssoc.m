...
nc = slideWindow; % prune null-hypothesis with length no less than nc
...
% remove tracks with only non-existence hypothesis
if ~isempty(trajectoryUpdMBM)
    aupd = [trajectoryUpdMBM.a]';
    Hupd = length(trajectoryUpdMBM);
    idx_remain = true(Hupd,1);
    for i = 1:Hupd
        if length(find(aupd==trajectoryUpdMBM(i).a))==1 && trajectoryUpdMBM(i).r==0
